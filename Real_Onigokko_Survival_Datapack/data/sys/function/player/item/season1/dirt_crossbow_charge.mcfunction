...
#スコアごとの処理
    ...
    execute if score @s dirt_crossbow_charge matches 40 run playsound item.crossbow.loading_end master @s
    execute if score @s dirt_crossbow_charge matches 40.. run clear @s dirt 1
    execute if score @s dirt_crossbow_charge matches 40.. if predicate sys:has_dirt_crossbow_main run item replace entity @s weapon.mainhand with crossbow[enchantments={"sys:dirt_crossbow":1,unbreaking:10},item_name={text:"土のクロスボウ",color:gold,italic:false},charged_projectiles=[{id:"dirt",count:1},{id:"dirt",count:1},{id:"dirt",count:1}],custom_data={dirt_crossbow:true},tooltip_display={hidden_components:["charged_projectiles"]}]
    execute if score @s dirt_crossbow_charge matches 40.. if predicate sys:has_dirt_crossbow_off run item replace entity @s weapon.offhand with crossbow[enchantments={"sys:dirt_crossbow":1,unbreaking:10},item_name={text:"土のクロスボウ",color:gold,italic:false},charged_projectiles=[{id:"dirt",count:1},{id:"dirt",count:1},{id:"dirt",count:1}],custom_data={dirt_crossbow:true},tooltip_display={hidden_components:["charged_projectiles"]}]
    execute if score @s dirt_crossbow_charge matches 40.. run attribute @s movement_speed modifier remove af68d8f1-2daa-4c66-b0af-8a1a8a34cd2b
    execute if score @s dirt_crossbow_charge matches 40.. run tag @s remove crossbow_charge
    execute if score @s dirt_crossbow_charge matches 40.. run scoreboard players set @s dirt_crossbow_charge 0
